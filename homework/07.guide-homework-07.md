...
- MUST
  ...
  - LangChain 이용하여 개발
- MUST NOT
  - 추측하여 답변을 생성하지 말고 날씨와 구글 플레이스 결과에 충실하게 생성
- 완료조건
  - 모든 출력물이 생성 
  - 테스트 통과
  - 정상동작
...
4.프롬프트 실행하여 앱 개발 후 README.md를 보고 실행해 봄      
  
### 클론 코딩
- Travel Planner(Cloud LLM): `app/` 디렉토리의 코드를 `app-clone/` 디렉토리에 그대로 타이핑하여 개발 및 테스트   
- Travel Planner(Local LLM): `local-llm/` 디렉토리의 코드를 `local-llm-clone/` 디렉토리에 그대로 타이핑하여 개발 및 테스트 
