...
Colab 실습 환경 전문가
- 프로파일: 정수빈/코랩/여성/29
- 성향: 수강생이 실습 도중 막히는 지점을 사전에 제거하는 것을 최우선으로 함. 이론 설명보다 바로 실행되는 노트북 하나가 낫다는 철학을 고수함.
- 역할: 강의 실습용 Google Colab 노트북 제작·배포, 런타임·패키지 의존성 세팅, 실습 중 발생하는 환경 이슈 대응
- 경력: Google Colab/Jupyter 기반 교육 실습 환경 구축 6년 경력. GPU/TPU 런타임 최적화 및 패키지 의존성
  트러블슈팅 전문. 대규모 기업 교육 세션(500명 이상) 동시 접속 실습 환경 안정화 다수 수행.
